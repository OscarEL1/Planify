      ...
      <main className="flex-1 px-8 py-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-[#64748B]">Información de tu cuenta y actividad</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-[#E4E7EC] rounded-2xl p-6 mb-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-xl font-bold text-[#1D2433]">{user?.name}</h2>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className="bg-white border border-[#E4E7EC] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              <ListChecks size={16} className="text-[#4F46E5]" />
              <span className="text-xs font-semibold text-[#64748B]">Total</span>
            </div>
            <p className="text-2xl font-bold text-[#1D2433]">{myActivities.length}</p>
          </div>
          <div className="bg-white border border-[#E4E7EC] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              <CheckCircle2 size={16} className="text-[#22C55E]" />
              <span className="text-xs font-semibold text-[#64748B]">Completadas</span>
            </div>
          ...
          <div className="bg-white border border-[#E4E7EC] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-semibold text-[#64748B]">En progreso</span>
            </div>
          ...
          <div className="bg-white border border-[#E4E7EC] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-semibold text-[#64748B]">Pendientes</span>
            </div>
        ...
        {myActivities.length > 0 && (
          <div className="bg-white border border-[#E4E7EC] rounded-2xl p-5 mb-6">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-[#1D2433]">Progreso general</h3>
              <span className="text-sm font-bold text-[#4F46E5]">
                {Math.round((completed / myActivities.length) * 100)}%
              </span>
            </div>
            <div className="w-full h-3 bg-[#E4E7EC] rounded-full overflow-hidden">
              <div
        ...
        <div className="bg-white border border-[#E4E7EC] rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-[#1D2433]">Mis actividades recientes</h3>
            <button
              type="button"
          ...
          {recentActivities.length === 0 ? (
            <p className="text-sm text-[#64748B]">No tienes actividades asignadas.</p>
          ) : (
            <div className="flex flex-col gap-3">
              {recentActivities.map((a) => {
                ...
                return (
                  <div
                    key={a.id}
                    className="flex items-center justify-between p-3 rounded-xl hover:bg-[#F8F9FB] cursor-pointer transition"
                    onClick={() => navigate(`/activities/${a.id}`)}
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: stColor }} />
                      <div>
                        <p className="text-sm font-medium text-[#1D2433]">{a.title}</p>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-xs text-[#64748B]">{STATUS_LABELS[a.status]}</span>
                          {a.dueDate && (
                            <>
