...
function DonutChart({ data, size = 140 }) {
  ...
  if (total === 0) return <div className="w-[140px] h-[140px] rounded-full border-8 border-[#E4E7EC]" />;
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox="0 0 120 120">
        <circle cx="60" cy="60" r={radius} fill="none" stroke="#E4E7EC" strokeWidth="12" />
        {segments.map((seg, i) => (
          <circle
            key={i}
            cx="60"
...
const progressPercent = stats?.completionPercentage ?? 0;
  ...
  const memberProgress = users?.map((u) => {
    const userActivities = activities?.filter((a) => a.assigneeId === u.id) || [];
    const userCompleted = userActivities.filter((a) => a.status === 'COMPLETADA').length;
    const userTotal = userActivities.length;
    const userPercent = userTotal > 0 ? Math.round((userCompleted / userTotal) * 100) : 0;
    return { ...u, total: userTotal, completed: userCompleted, percent: userPercent };
  }).filter((u) => u.total > 0).sort((a, b) => b.percent - a.percent) || [];
      ...
      <main className="flex-1 px-8 py-10">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          <div className="bg-white border border-[#E4E7EC] rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-semibold text-[#64748B] uppercase tracking-wide">Total de actividades</p>
          ...
          <div className="bg-white border border-[#E4E7EC] rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-semibold text-[#64748B] uppercase tracking-wide">Completadas</p>
            ...
            <p className="text-xs text-[#64748B] mt-1">de {total} actividades</p>
          </div>

          <div className="bg-white border border-[#E4E7EC] rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-semibold text-[#64748B] uppercase tracking-wide">En proceso</p>
          ...
          <div className="bg-white border border-[#E4E7EC] rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-semibold text-[#64748B] uppercase tracking-wide">Tasa de finalización</p>
              <div className="w-8 h-8 rounded-lg bg-[#EEF2FF] flex items-center justify-center">
            ...
            <div className="flex items-center gap-3">
              <div className="relative w-12 h-12">
                <svg width="48" height="48" viewBox="0 0 48 48">
                  <circle cx="24" cy="24" r="20" fill="none" stroke="#E4E7EC" strokeWidth="4" />
                  <circle
                    cx="24" cy="24" r="20" fill="none" stroke="#4F46E5" strokeWidth="4"
              ...
              <div>
                <p className="text-2xl font-bold text-[#1D2433]">{progressPercent}%</p>
                <p className="text-xs text-[#64748B]">de todas las actividades</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white border border-[#E4E7EC] rounded-2xl p-6">
            <h2 className="text-sm font-semibold text-[#1D2433] mb-6">Actividades por estado</h2>
              ...
              <div className="flex flex-col gap-3">
                {donutData.map((d) => (
                  <div key={d.label} className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-[#1D2433] w-24">{d.label}</span>
                    <span className="text-sm font-semibold text-[#1D2433] w-6 text-right">{d.value}</span>
          ...
          <div className="bg-white border border-[#E4E7EC] rounded-2xl p-6">
            <h2 className="text-sm font-semibold text-[#1D2433] mb-6">Progreso por integrante</h2>
            <div className="flex flex-col gap-4">
              {memberProgress.length === 0 && (
                <p className="text-sm text-[#64748B]">No hay actividades asignadas.</p>
              )}
                ...
                <div key={member.id}>
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-[#1D2433]">{member.name}</span>
                    </div>
                    <span className="text-sm font-bold text-[#1D2433]">{member.percent}%</span>
                  </div>
                  <div className="flex items-center gap-2">
        ...
        {recentActivities.length > 0 && (
          <div className="bg-white border border-[#E4E7EC] rounded-2xl p-6">
            <h2 className="text-sm font-semibold text-[#1D2433] mb-4">Actividades recientes</h2>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-[#1D2433]">{activity.title}</p>
                        <p className="text-xs text-[#64748B]">
                          {activity.assignee?.name || 'Sin asignar'}
                          {activity.dueDate && ` · Vence el ${formatDate(activity.dueDate)}`}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className="text-xs font-medium px-2 py-0.5 rounded-full"
                        style={{ backgroundColor: `${statusColors[activity.status]}20`, color: statusColors[activity.status] }}
                      >
